 ...
 */
public class TouchCommandHandler extends CommandHandler
{

    private static final String lineSeparator = System.getProperty("line.separator");
    
    // command usage
    private static final String cmdUsage = "pwd: usage:"+lineSeparator+" touch [target]"+lineSeparator
            + "  options: not admitted"+lineSeparator
            + "  target: mandatory"+lineSeparator
            + "    'stuff.blah'    to create the file named stuff.blah"+lineSeparator;
    // command pattern checker
    private static final String RES_PATTERN = "([a-zA-Z0-9]+\\.{1}[a-z0-9]+)";

    public TouchCommandHandler() 
    {
        super();
        cmdName = "touch";
        cmdDescription = "Create a file resource locally (with respect to the working"
                + " directory).";
    }
    ...
    public ExecutionReport handleCommand(ClientCommand clientQuery, ServerSession clientSess) 
    {
        ...
        String errorMsg = "";
        if(clientQuery.getCmd().equalsIgnoreCase("touch")) {
            ...
            File fToCreate = new File(newResource);
            if(fToCreate.exists()) {
                // an error occurred
                report.setSuccess(true);
                report.setExecutionOutput("telnet: "+newResource+" already exists: misuse");
                return report;
            }
            try {
                fToCreate.createNewFile();
            } catch (IOException ex) {
                // an error occurred
                report.setSuccess(false);
                report.setExecutionOutput("telnet: error occurred in creating the file: retry");
                return report;
            }
            report.setSuccess(true);
            report.setExecutionOutput(newResource);
            return report;
